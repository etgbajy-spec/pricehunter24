...
### 요약 설명 (132자 이내)
```
쿠팡·네이버·G마켓 상품 페이지 우측 하단 「구매판단 의뢰」 버튼으로 가격·옵션을 읽어 의뢰 폼을 자동 입력합니다.
```

### 상세 설명 (예시)
```
PriceHunter 크롬 확장 프로그램은 쇼핑몰 상품 페이지에서 링크·상품명·가격·선택 옵션을 읽어,
구매판단 리포트 의뢰 페이지를 자동으로 채워 줍니다.

【설치 후 바로 보이는 버튼】
상품 페이지를 열면 화면 우측 하단에 「🎯 구매판단 의뢰」 플로팅 버튼이 나타납니다.
옵션을 선택한 뒤 이 버튼을 누르거나, 툴바의 PriceHunter 아이콘을 눌러도 됩니다.

【지원 쇼핑몰】
· 쿠팡 · 네이버(스마트스토어·브랜드스토어) · 11번가 · G마켓 · 옥션

【사용 방법 — 3단계】
1. Chrome 웹스토어에서 PriceHunter를 설치합니다.
2. 쇼핑몰 상품 페이지에서 원하는 옵션을 선택합니다.
3. 우측 하단 「구매판단 의뢰」 버튼(또는 툴바 아이콘)을 클릭합니다.
   → 의뢰 페이지가 열리며 링크·상품명·가격·옵션이 자동 입력됩니다.
...
【개인정보처리방침】
https://pricehunt24.com/extension-privacy.html
```
...
### 「개인정보처리방침 링크가 유효하지 않습니다」

- `#chrome-extension` 같은 **앵커(#) URL은 거부되는 경우가 많습니다.**
- 아래 URL만 사용하세요 (배포 후 브라우저에서 열어 확인):

```
https://pricehunt24.com/extension-privacy.html
```

### 「게시자 연락처 이메일을 인증해야 합니다」 / 「연락 가능한 이메일을 입력해야 합니다」

코드로 해결 불가 — **개발자 콘솔 설정**에서 처리:

1. https://chrome.google.com/webstore/devconsole 접속
2. 왼쪽 하단 **설정**(톱니바퀴) 또는 **계정** 메뉴
3. **게시자 연락처 이메일**에 `pricehunter.service@gmail.com` 등 실제 수신 가능한 주소 입력
4. **인증 메일 보내기** → Gmail 받은편지함에서 링크 클릭
5. 인증 완료 후 다시 **심사 제출**

> 개발자 등록($5)과 **게시자 이메일 인증**은 별개입니다. 둘 다 완료해야 게시됩니다.
