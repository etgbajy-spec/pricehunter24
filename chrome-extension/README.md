...
3. 우측 하단 버튼이 없거나 자동 수집이 안 되면 확장 아이콘 → **직접 입력하러 가기**
4. `request-lite.html`이 열리며 **링크·상품명·가격·옵션**이 자동 입력됨 (옵션은 의뢰 페이지에서 확인·수정)
5. 이메일 입력 후 의뢰
...
`lib/config.js`의 `DEFAULT_SITE`를 `http://localhost:1000`으로 바꾼 뒤 `npm start`로 서버 실행.
